...
async function fetchData(url: string): Promise<Report[]> {
  const response = await fetch(url);
  const data: Report[] = await response.json();
  return data;
}

async function generateReport(url: string, targetDate: string): Promise<void> {
  try {
    const data = await fetchData(url);

    // Find the report with the target date
    const report = data.find(report => report.report_date === targetDate);

    if (report) {
      const killed = report.killed;
      const killedCum = report.killed_cum;

      // Generate the formatted text
      const finalText = `Today ${targetDate}, Israel killed ${killed} people in the last 24 hours. The total number of killed in this genocide were ${killedCum}.`;

      console.log(finalText);
    } else {
      console.log(`No report found for the date: ${targetDate}`);
    }
  } catch (error) {
...
// Example usage
const url = 'https://data.techforpalestine.org/api/v2/casualties_daily.json'; // Replace with your JSON URL
const targetDate = '2024-08-08';
generateReport(url, targetDate);