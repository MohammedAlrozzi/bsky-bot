...
const fetchData = async (url: string): Promise<Report[]> => {
...
const getPostText = async (url: string, targetDate: string): Promise<string> => {
  try {
    ...
    if (report) {
      ...
      return `Today ${targetDate}, Israel killed ${killed} people in the last 24 hours. The total number of killed in this genocide were ${killedCum}.`;
    } else {
      return `No report found for the date: ${targetDate}`;
    }
  } catch (error) {
